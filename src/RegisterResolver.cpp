...
std::vector<RegisterDefinition> RegisterResolver::resolveFromRanges(const std::vector<std::string>& ranges) {
    ...
    for (const std::string& range : ranges) {
        ...
        for (uint16_t address : addresses) {
            ...
            if (regDef != nullptr) {
                ...
                std::cerr << "Warning: Register address " << address << " not found in device config" << std::endl;
            }
        }
    }
...
std::vector<RegisterDefinition> RegisterResolver::resolveFromFile(const std::string& filePath) {
...
std::vector<RegisterDefinition> RegisterResolver::resolveFromAddresses(const std::vector<uint16_t>& addresses) {
    std::vector<RegisterDefinition> result;
...
std::vector<uint16_t> RegisterResolver::parseRange(const std::string& range) {
    ...
    if (dashPos != std::string::npos) {
        ...
        try {
            ...
            if (start > end) {
                std::cerr << "Warning: Invalid range " << range << " (start > end), skipping" << std::endl;
                return {};
            }
            ...
            std::cerr << "Warning: Invalid range format " << range << ", skipping" << std::endl;
            return {};
        }
    }
    ...
    try {
        ...
        std::cerr << "Warning: Invalid address format " << range << ", skipping" << std::endl;
        return {};
    }
}

std::vector<uint16_t> RegisterResolver::parseCommaSeparated(const std::string& addresses) {
    ...
    while (std::getline(iss, token, ',')) {
        ...
        if (!token.empty()) {
            try {
                ...
                std::cerr << "Warning: Invalid address format " << token << ", skipping" << std::endl;
            }
        }
    }
